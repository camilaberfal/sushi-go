...
import wasabiWithoutPlateImg from "@/app/assets/illustrations/wasabi-without-plate.png";
import { getMakiIconsTotal, scoreMakiForPlayers } from "@/domain/scoring";
...
const CAT_INFO: Record<string, { label: string; img: any }> = {
  "Makis": { label: "Makis", img: makisX3Img },
...
type CollectionStat = { count: number; points: number; boostedCount: number };

function cardTypeFromCardId(cardId: string): CardType | null {
  const base = cardId.split("-")[0] as CardType;
  ...
  return valid.includes(base) ? base : null;
}

function categoryFromBase(base: string): string {
  if (base === "maki_1" || base === "maki_2" || base === "maki_3") return "Makis";
...
function buildCollectionStats(cardIds: string[], liveMakiPoints: number): Record<string, CollectionStat> {
  ...
  for (const cardId of cardIds) {
    ...
    if (!stats[category]) {
      stats[category] = { count: 0, points: 0, boostedCount: 0 };
    }
  ...
  if (!stats["Makis"]) {
    stats["Makis"] = { count: 0, points: 0, boostedCount: 0 };
  }
  stats["Makis"].points = liveMakiPoints;
...
export function GlassScoreboard({
  ...
  const allPlayersCards = [myPlayedCards, ...others.map((player) => player.playedCards ?? [])];
  const makiIconsByPlayer = allPlayersCards.map((cards) => {
    const types = cards
      .map((cardId) => cardTypeFromCardId(cardId))
      .filter((type): type is CardType => type !== null);
    return getMakiIconsTotal(types);
  });
  const liveMaki = scoreMakiForPlayers(makiIconsByPlayer).pointsByPlayer;
  const myLiveMakiPoints = liveMaki[0] ?? 0;
  const rivalMakiPointsById = Object.fromEntries(
    others.map((player, index) => [player.id, liveMaki[index + 1] ?? 0])
  ...
  return (
    <div className="fixed bottom-0 right-0 top-0 z-40 w-[360px] bg-[#1a0f14] border-l-4 border-l-[#251319] shadow-[-8px_0_25px_rgba(0,0,0,0.8)] overflow-hidden flex flex-col font-sans text-white border-0">
      ...
      <div className="relative z-10 flex h-full min-h-0 flex-col p-4">
        ...
        <div className="min-h-0 flex-1 overflow-y-auto pr-1 [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-white/20 [&::-webkit-scrollbar-track]:bg-black/20">
          ...
          {myPlayedCards.length > 0 && (
            <div className="mb-4 relative">
              ...
              <div className="flex flex-col gap-2 pr-1">
                {Object.entries(buildCollectionStats(myPlayedCards, myLiveMakiPoints))
                .sort(([, statA], [, statB]) => {
                  ...
                  return (
                    <div key={cat} className={`flex items-center justify-between rounded-xl p-2 shadow-[0_6px_10px_rgba(0,0,0,0.5),inset_0_2px_4px_rgba(255,255,255,0.02),inset_0_-2px_6px_rgba(0,0,0,0.6)] transition-colors border max-h-[52px] ${tintColors}`}>
                      ...
                      <div className="flex items-center gap-1.5">
                        <div className="h-8 w-[66px] overflow-hidden px-1.5 py-1 bg-[#0a0406] rounded-lg shadow-[inset_0_3px_5px_rgba(0,0,0,0.8)] flex flex-wrap content-center items-center justify-center gap-1">
                          {Array.from({ length: stat.count }).map((_, dotIndex) => {
                            const wasabiMarker = cat.toLowerCase().includes("nigiri") && dotIndex < stat.boostedCount;
                            return wasabiMarker ? (
                              <Image
                                key={`${cat}-marker-${dotIndex}`}
                                src={wasabiWithoutPlateImg}
                                alt="W"
                                width={8}
                                height={8}
                              ...
                              <span
                                key={`${cat}-marker-${dotIndex}`}
                                className="h-1.5 w-1.5 rounded-full bg-[#fbbf24] shadow-[0_0_6px_rgba(251,191,36,0.7)]"
                              />
                            );
                          })}
                        </div>
                        <div className="h-8 w-[58px] px-2 py-0.5 bg-[#0a0406] rounded-lg shadow-[inset_0_3px_5px_rgba(0,0,0,0.8)] flex items-center justify-center">
          ...
          <div className="mt-1 flex min-h-0 flex-col">
            ...
            <div className="space-y-2.5 pb-1">
              ...
              {others.map((player) => (
                ...
                >
                  {(() => {
                    const livePuddings = (player.playedCards ?? []).filter((cardId) => cardTypeFromCardId(cardId) === "pudding").length;
                    const puddingCount = player.playedCards ? livePuddings : player.puddings;
                      ...
                      <div className="flex flex-col justify-center">
                        ...
                        <div className="flex gap-1 items-center mt-0.5">
                          <div className="flex items-center gap-1 bg-[#0a0406] px-1.5 py-0.5 rounded shadow-[inset_0_2px_4px_rgba(0,0,0,0.8)]">
                            ...
                            <span className="text-sm font-bold text-[#f9a8d4] leading-none mb-0.5 mt-0.5">{puddingCount}</span>
                          </div>
                          <div className="flex items-center gap-1 bg-[#0a0406] px-1.5 py-0.5 rounded shadow-[inset_0_2px_4px_rgba(0,0,0,0.8)]">
                          ...
                          <div className="flex items-center gap-1 bg-[#0a0406] px-1.5 py-0.5 rounded shadow-[inset_0_2px_4px_rgba(0,0,0,0.8)]">
                            <Image src={makisX3Img} alt="Maki" width={10} height={10} className="object-contain" />
                            <span className="text-sm font-bold text-[#fde68a] leading-none mb-0.5 mt-0.5">{rivalMakiPointsById[player.id] ?? 0}</span>
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        <div className="mt-3 shrink-0 pt-2 relative z-20 flex flex-col gap-2">
